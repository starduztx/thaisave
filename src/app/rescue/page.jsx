      ...
      <nav className="bg-[#1E3A8A] text-white w-full shadow-md sticky top-0 z-50">
        <div className="w-full px-6 py-4 flex justify-between items-center">
          <div className="flex flex-col">
            <Link href="/" className="text-2xl font-bold tracking-tight hover:opacity-90">ThaiSave(ไทยเซฟ)</Link>
            <span className="text-[11px] text-blue-200 font-light tracking-widest opacity-80">ระบบกลางจัดการภัยพิบัติแห่งชาติ</span>
          </div>
          <div className="hidden md:flex items-center gap-8 text-sm font-medium">
            <span className="text-yellow-400 font-bold border-b-2 border-yellow-400 pb-1">ช่วยเหลือ/กู้ภัย</span>
            <button onClick={handleLogout} className="text-white hover:text-white/80 bg-red-500 hover:bg-red-600 px-4 py-1.5 rounded-lg">ออกจากระบบ</button>
          </div>
        </div>
      </nav>
                    ...
                    <div className="p-6">
                        ...
                        <div className="flex flex-col md:flex-row gap-3 pt-4 border-t border-gray-100">
                            ...
                            {(item.status === 'accepted' || item.status === 'traveling') && (
                                <button onClick={() => setChatReport(item)} className="flex-none px-3 bg-blue-50 text-blue-600 border border-blue-200 rounded-lg hover:bg-blue-100 transition">
